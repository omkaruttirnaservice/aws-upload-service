...
exports.getProcess = async () => {
    return await this.getFromGlobalCache(`p_${process.env.DB_DATABASE}`);
};
...
exports.isDateExpired = (datesList, type) => {
    ...
    if (tempDateDetails.length > 0) {
        return tempDateDetails[0]?.isExpired ? tempDateDetails[0].isExpired : false;
    } else return false;
};
