...
const awsController = {
    uploadFile: async (req, res) => {
        try {
            ...
            console.log(uploadFile, '==uploadImg==');
            ...
            let { bucketName, fileName, folderName, fileExtension, extras } = req.body;

            // const imageName = `${app_id}_${img_type}_${momentDates.getTimestamp()}.${fileExtension}`;
            // ${folderName}/

            if (extras.appId && extras.imgType) {
                fileName = `${extras.appId}_${extras.appId}_${momentDates.getTimestamp()}.${fileExtension}`;
            } else {
                fileName = `${fileName}_${momentDates.getTimestamp()}.${fileExtension}`;
            }

            let fullImgName = `${folderName}/${fileName}`;
